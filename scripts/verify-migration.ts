// Verification: reads the live Supabase DB via the app's own pipeline and
// prints the leaderboard so it can be eyeballed against the pre-migration
// numbers (mirrors the old scripts/verify-import.ts).
import { loadEnvConfig } from "@next/env";
loadEnvConfig(process.cwd());

import("../lib/db").then(async ({ computePlayerStats }) => {
